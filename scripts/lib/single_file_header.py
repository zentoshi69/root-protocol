...
def main() -> int:
    ...
    def render(offset: int) -> str:
        ...
        w("//   * Paid settlement applies 50/25/25 floor shares and assigns the integer remainder to")
        w("//     protocol, so every wei is conserved with no stranded or double-counted dust.")
        w("//   * Refunds, withdrawals and terminal resolution of active BTC reservations remain")
        w("//     available while paused. Pausing stops new obligations; it never traps an existing one.")
